...
- (id)initWithSize:(CGSize)framebufferSize;
{
  ...
  if (!(self = [self initWithSize:framebufferSize
...
- (void)generateFramebuffer;
{
  runSynchronouslyOnVideoProcessingQueue(^{
    ...
    // By default, all framebuffers on iOS 5.0+ devices are backed by texture
    // caches, using one shared cache
    if ([GPUImageContext supportsFastTextureUpload]) {
      ...
      empty = CFDictionaryCreate(
          ...
          &kCFTypeDictionaryValueCallBacks); // our empty IOSurface properties
                                             // dictionary
      ...
      CVReturn err = CVPixelBufferCreate(
          kCFAllocatorDefault, (int)_size.width, (int)_size.height,
    ...
    // 将纹理附着在帧缓冲区中,mipmap的level是0
      ...
      glTexImage2D(GL_TEXTURE_2D, 0, _textureOptions.internalFormat,
                   (int)_size.width, (int)_size.height, 0,
                   _textureOptions.format, _textureOptions.type, 0);
      glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
    ...
    NSAssert(status == GL_FRAMEBUFFER_COMPLETE, @"Incomplete filter FBO: %d",
             status);
#endif
...
- (void)unlock;
{
  ...
  NSAssert(framebufferReferenceCount > 0,
           @"Tried to overrelease a framebuffer, did you forget to call "
           @"-useNextFrameForImageCapture before using "
...
void dataProviderUnlockCallback(void *info, const void *data, size_t size) {
  GPUImageFramebuffer *framebuffer =
      (__bridge_transfer GPUImageFramebuffer *)info;

  [framebuffer restoreRenderTarget];
  [framebuffer unlock];
  [[GPUImageContext sharedFramebufferCache]
      removeFramebufferFromActiveImageCaptureList:framebuffer];
}

- (CGImageRef)newCGImageFromFramebufferContents;
{
  // a CGImage can only be created from a 'normal' color texture
  NSAssert(self.textureOptions.internalFormat == GL_RGBA,
           @"For conversion to a CGImage the output texture format for this "
           @"filter must be GL_RGBA.");
  NSAssert(self.textureOptions.type == GL_UNSIGNED_BYTE,
           @"For conversion to a CGImage the type of the output texture of "
           @"this filter must be GL_UNSIGNED_BYTE.");
  ...
  runSynchronouslyOnVideoProcessingQueue(^{
    ...
    // It appears that the width of a texture must be padded out to be a
    // multiple of 8 (32 bytes) if reading from it using a texture cache
    ...
    if ([GPUImageContext supportsFastTextureUpload]) {
#if TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE
      NSUInteger paddedWidthOfImage =
          CVPixelBufferGetBytesPerRow(renderTarget) / 4.0;
      NSUInteger paddedBytesForImage =
          paddedWidthOfImage * (int)_size.height * 4;

      glFinish();
      CFRetain(renderTarget); // I need to retain the pixel buffer here and
                              // release in the data source callback to prevent
                              // its bytes from being prematurely deallocated
                              // during a photo write operation
      [self lockForReading];
      rawImagePixels = (GLubyte *)CVPixelBufferGetBaseAddress(renderTarget);
      dataProvider = CGDataProviderCreateWithData(
          (__bridge_retained void *)self, rawImagePixels, paddedBytesForImage,
          dataProviderUnlockCallback);
      [[GPUImageContext sharedFramebufferCache]
          addFramebufferToActiveImageCaptureList:self]; // In case the
                                                        // framebuffer is
                                                        // swapped out on the
                                                        // filter, need to have
                                                        // a strong reference to
                                                        // it somewhere for it
                                                        // to hang on while the
                                                        // image is in existence
#else
#endif
    } else {
      ...
      // 将纹理读取到内存空间
      glReadPixels(0, 0, (int)_size.width, (int)_size.height, GL_RGBA,
    ...
    if ([GPUImageContext supportsFastTextureUpload]) {
      ...
      cgImageFromBytes = CGImageCreate(
          (int)_size.width, (int)_size.height, 8, 32,
          CVPixelBufferGetBytesPerRow(renderTarget), defaultRGBColorSpace,
      ...
      cgImageFromBytes = CGImageCreate(
          (int)_size.width, (int)_size.height, 8, 32, 4 * (int)_size.width,
          defaultRGBColorSpace, kCGBitmapByteOrderDefault | kCGImageAlphaLast,
          dataProvider, NULL, NO, kCGRenderingIntentDefault);
    }
...
#if TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE
  if ([GPUImageContext supportsFastTextureUpload]) {
    NSAssert(readLockCount > 0,
             @"Unbalanced call to -[GPUImageFramebuffer unlockAfterReading]");
    readLockCount--;
    if (readLockCount == 0) {
...
#else
    return _size.width *
           4;  // TODO: do more with this on the non-texture-cache side
#endif
  } else {
